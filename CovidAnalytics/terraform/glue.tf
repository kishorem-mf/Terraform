resource "aws_s3_object" "covid_data_processing_script" {
  bucket = var.s3_bucket_covid
  key = "glue/scripts/CovidDataProcessing.py"
  source = "${local.glue_src_path}CovidDataProcessing.py"
  etag = filemd5("${local.glue_src_path}CovidDataProcessing.py")
}

resource "aws_glue_job" "covid_data_processing" {
  glue_version = "4.0" #optional
  max_retries = 0 #optional
  name = "CovidDataProcessing" #required
  description = "Covid data processing with aws glue service and terraform" #description
  ...
  tags = {
    project = var.projectcovid #optional
  }
  command {
    name="glueetl" #optional
    script_location = "s3://${var.s3_bucket_covid}/glue/scripts/CovidDataProcessing.py" #required

  }
  default_arguments = {
    "--s3_bucket"               = var.s3_bucket_covid
    "--s3_filepath"             = var.s3_filepath
    "--class"                   = "GlueApp"
